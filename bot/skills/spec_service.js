...
var _bdd_constructions = require("../lib/specs/bdd_constructions");

var _step = require("../lib/specs/step");
...
class SpecService extends _skills_base2.default {
  ...
  async build_spec() {
    ...
    let spec = _spec.Spec.new_service_validation();
    ...
    while (idx > -1) {
      ...
      if (idx < N - 1) {
        ...
        let klass = await _klass.Klass.new_model(model_name);
        instances.push(klass.new_instance(inst_name));
      }
      idx = this.entities.findIndex((o, index, __) => o.entity === 'model_name' && index > idx);
    }
    ...
    while (idx_method > -1) {
      ...
      let idx_method2 = this.entities.findIndex((obj, index, _) => obj.entity === KEY && index > idx_method || obj.entity === "service_name");
      let instance = null;
      ...
      if (instance) {
        let step = _step.Step.new_method(instance, method);
        //parse params from when
        let idx_param = idx;
        while (true) {
          idx_param = this.entities.findIndex((obj, index, _) => idx_param < index && index < idx_method2 && obj.entity === "sym_value");
    ...
    let service_name = this.entities[idx].value;
    let service = await _klass.Klass.new_model(service_name);
    let instance_service = service.new_instance(service_name);
    let expectation = _expectation.Expectation.new_instance_method(instance_service, "run");
  ...
  constructor(controller, stack) {
    ...
    controller.studio.beforeThread(this.script_name(), 'ask_requirement', (convo, next) => {
